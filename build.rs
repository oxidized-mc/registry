...
fn main() {
    ...
    let items_path = manifest_path.join("src/data/items.json.gz");

    // Vanilla tags — bundled with registry crate
    let tags_path = manifest_path.join("src/data/tags.json");
    ...
    // ── Generate item IDs ───────────────────────────────────────────────
    let item_dest = Path::new(&out_dir).join("item_ids_generated.rs");
    let item_code = generate_item_ids(&items_path);
    fs::write(&item_dest, &item_code).expect("Failed to write item IDs generated code");
}
...
/// Generates `item_ids_generated.rs` containing static item lookup tables.
///
/// Reads `items.json.gz`, extracts item names, max stack sizes, and max damage
/// values, and produces:
/// - `ITEM_COUNT` — total number of items
/// - `ITEM_NAMES` — names in vanilla registration order (index = protocol ID)
/// - `ITEM_MAX_STACK_SIZES` — max stack size per item
/// - `ITEM_MAX_DAMAGES` — max durability per item (0 = no durability)
/// - `ITEM_NAMES_SORTED` — alphabetically sorted (name, index) for binary search
fn generate_item_ids(items_path: &Path) -> String {
    let compressed = fs::read(items_path)
    ...
    let mut items: Vec<ItemData> = Vec::with_capacity(obj.len());
    for (name, value) in obj {
        let raw_stack = value
            .get("max_stack_size")
            .and_then(|v| v.as_u64())
    ...
    // Sorted lookup table: (name, registration-order index)
    let mut sorted_lookup: Vec<(&str, u16)> = items
        .iter()
        .enumerate()
        .map(|(i, item)| (item.name.as_str(), i as u16))
        .collect();
    sorted_lookup.sort_by_key(|&(n, _)| n);

    // ── Write generated code ────────────────────────────────────────────
    let mut out = String::with_capacity(256 * 1024);

    let _ = writeln!(out, "// @generated by build.rs — do not edit\n");
    let _ = writeln!(out, "/// Total number of item types.");
    let _ = writeln!(out, "pub const ITEM_COUNT: usize = {};\n", items.len());
    ...
    for chunk in items.chunks(20) {
        ...
        for (i, item) in chunk.iter().enumerate() {
            ...
            let _ = write!(out, "{}", item.max_stack_size);
        }
        let _ = writeln!(out, ",");
    }
    let _ = writeln!(out, "];\n");
    ...
    for chunk in items.chunks(20) {
        ...
        for (i, item) in chunk.iter().enumerate() {
            ...
            let _ = write!(out, "{}", item.max_damage);
        }
        let _ = writeln!(out, ",");
    }
    let _ = writeln!(out, "];\n");
    ...
    let _ = writeln!(
        out,
        "/// Item names sorted alphabetically for binary search lookup."
    );
    let _ = writeln!(
        out,
        "pub static ITEM_NAMES_SORTED: [(&str, u16); ITEM_COUNT] = ["
    );
    for &(name, idx) in &sorted_lookup {
        let _ = writeln!(out, "    ({:?}, {idx}),", name);
    }
    let _ = writeln!(out, "];");
